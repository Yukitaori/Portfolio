  ...
  useEffect(() => {
    ...
    if (formSubmit && !error) {
      emailjs.send('service_hatoyzi', 'contact_form', templateParams)
        .then((response) => {
          setInformationMessage('Your message has been sent successfully. Thanks!');
          setName('');
          setMail('');
          setMessage('');
          setFormSubmit();
  ...
  return (
    <div id="contact">
      ...
      <form>
        ...
        {informationMessage ? <p className={styles.errorMessage}>{informationMessage}</p> : null}
        <button type="submit" className={styles.submitButton} disabled={name === '' || mail === '' || message === ''} onClick={(e) => handleSubmit(e)}> Submit</button>
      </form>
    </div>
  );
}
